...
class HabitTracker(hass.Hass):
    ...
    def _schedule_end_of_day_reminder(
        ...
        if (
            ...
            or not self._is_required_today(user, config.slot)
        ):
        ...
        for kind, template in templates:
            ...
            if not known:
                self.error(
                    ...
                    "so it will only re-evaluate at startup, midnight, configuration "
                    "changes, and before reminders: %r",
                    kind,
                    user,
                    slot,
                    template,
    ...
    def _send_end_of_day_reminder(self, user: str, slot: int) -> None:
        ...
        if (
            ...
            or not config.configured
            or not config.end_of_day_reminder_enabled
            or not self._evaluate_requirement(user, slot, manage_reminders=False)
            or self._is_complete_today(user, slot)
    ...
    def _midnight_rollover(self, kwargs: dict[str, Any]) -> None:
        ...
        for slot, config in data.habits.items():
            if config.configured:
                self._rebuild_template_listeners(user, slot)
                self._evaluate_requirement(user, slot, manage_reminders=False)
                if self._is_required_today(user, slot):
                    self._seed_next_reminder(
                        user,
                        config,
                        force=True,
